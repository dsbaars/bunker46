...
async function importConnection() {
  ...
  try {
    ...
    await api.post('/connections', {
      ...
      logoUrl: parsed.image || undefined,
  ...
  <div>
    <div class="flex items-center justify-between mb-6">
      ...
      <div v-if="mode === 'idle'" class="flex gap-2">
        <Button @click="mode = 'generate'"> Generate bunker:// URI </Button>
        <Button variant="ghost" @click="mode = 'import'"> Import URI </Button>
      </div>
        ...
        <div>
          ...
          >
            ...
            <option v-for="key in nsecKeys" :key="key.id" :value="key.id">
              {{ key.label }} ({{ key.publicKey.slice(0, 16) }}...)
            </option>
          </select>
        </div>
        <div>
    ...
    <Card v-if="mode === 'import'" class="mb-6">
      <h2 class="text-lg font-semibold mb-2">Import Connection URI</h2>
      <p class="text-sm text-muted-foreground mb-4">
